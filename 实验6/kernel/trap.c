...
//检查 scause 寄存器。如果值为 3 (Breakpoint)，说明这是我们模拟的系统调用。
...
//通过 sp_val 指针，将栈上的寄存器值复制到当前进程 PCB 中的 p->trapframe 结构里
//模拟了用户栈 -> 内核栈时Trapframe 的作用
        ...
        // w_sepc(sepc + 4); 
        //处理pc指针，因为异常发生在 ebreak 指令处，返回时必须跳过这条指令，否则会无限循环执行断点。
