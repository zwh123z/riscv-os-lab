...
void kvminit(void) {
    ...
    // 映射 UART 设备，用于printf输出
    mappages(kernel_pagetable, 0x10000000, PGSIZE, 0x10000000, PTE_R | PTE_W);

    // 映射内核代码段 (R-X)
    mappages(kernel_pagetable, 0x80000000, (uint64)etext - 0x80000000, 0x80000000, PTE_R | PTE_X);
